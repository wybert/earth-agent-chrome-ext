...
export function SessionSidebar({
  ...
  const saveEditing = () => {
    if (editingSessionId && editingTitle.trim() && editingTitle.trim() !== sessions.find(s => s.id === editingSessionId)?.title) {
      ...
      >
        ...
        {!isEditing && (
          <div className="mt-2 flex flex-wrap items-center gap-1 opacity-0 transition group-hover:opacity-100">
            <Button variant="ghost" size="icon" className="h-7 w-7" title="Rename" onClick={(event) => { event.stopPropagation(); startEditing(session) }}>
