...
if ((window as any)[CONTENT_SCRIPT_ID]) {
  ...
  console.log(
    `⚠️ [Content Script] Another instance already running (timestamp: ${existingTimestamp}). Current timestamp: ${INSTANCE_TIMESTAMP}.`
  );

  // If this instance is newer, take over
  if (INSTANCE_TIMESTAMP > existingTimestamp) {
    console.log(
      `✅ [Content Script] This instance is newer, taking over from old instance ${existingTimestamp}`
    );
    ...
    console.log(
      `🛑 [Content Script] Exiting: older/duplicate instance (${INSTANCE_TIMESTAMP} <= ${existingTimestamp})`
    );
    shouldExecute = false;
 ...
 */
async function getEditorContentViaPageContext(): Promise<{
  success: boolean;
  content?: string;
  error?: string;
}> {
 ...
 */
async function setEditorContentViaPageContext(
  content: string
...
function findMainAceEditor(): { editor: any; session: any } | null {
  ...
  console.warn(
    '[findMainAceEditor] Could not find Ace editor (expected in isolated content script world)'
  );
  return null;
}
...
function registerEditorShadowSync() {
  ...
  try {
    const editorPaths = [
      () =>
        Array.from(document.querySelectorAll('.ace_editor'))
          .map((el) => (el as any).__ace_editor__ || (el as any).env?.editor)
          .find((editor) => editor),
      () => (window as any).code?.editor?.aceEditor,
      () => (window as any).ee?.Editor?.ace,
    ];
      ...
      (response) => {
        if (chrome.runtime.lastError) {
          ...
          if (notificationRetries < MAX_NOTIFICATION_RETRIES) {
            ...
            console.log(
              `Retrying notification in ${delay}ms (attempt ${notificationRetries}/${MAX_NOTIFICATION_RETRIES})...`
            );
...
if (shouldExecute && !messageListenerAdded) {
  console.log(
    `🔧 [Content Script][${INSTANCE_TIMESTAMP}] Adding message listener (single instance)`
  );
  ...
  chrome.runtime.onMessage.addListener((message: Message, sender, sendResponse) => {
    ...
    if (!shouldExecute || globalShouldExecute === false || !isActiveInstance) {
      console.log(
        `🛑 [Content Script][${INSTANCE_TIMESTAMP}] Instance deactivated, ignoring message: ${message.type}`
      );
    ...
    if (isContextInvalidated) {
      console.error(
        'Content script context is invalidated. Aborting message handling for:',
        message.type
      );
      ...
      switch (message.type) {
        ...
        case 'GET_ELEMENT_BY_REF_ID':
          if (message.payload && message.payload.refId) {
            ...
            sendResponse({
              success: false,
              error: 'refId not provided in payload for GET_ELEMENT_BY_REF_ID',
            });
          }
          return true; // Will respond asynchronously
        ...
        case 'CLICK_BY_COORDINATES':
          if (
            ...
            sendResponse({
              success: false,
              error: 'Invalid payload for CLICK_BY_COORDINATES: x and y are required.',
            });
          }
          return true; // Will respond asynchronously
        ...
        case 'CLICK_BY_SELECTOR':
          if (message.payload && typeof message.payload.selector === 'string') {
            handleClickBySelector(
              message.payload.selector,
              message.payload.elementDescription,
              sendResponse
            );
          } else {
            sendResponse({
              success: false,
              error: 'selector not provided in payload for CLICK_BY_SELECTOR',
            });
          }
          return true; // Will respond asynchronously
        ...
        case 'CLICK_BY_REF_ID':
          if (message.payload && message.payload.refId) {
            ...
            sendResponse({
              success: false,
              error: 'refId not provided in payload for CLICK_BY_REF_ID',
            });
          }
          return true; // Will respond asynchronously
  ...
  console.log(
    `🛑 [Content Script][${INSTANCE_TIMESTAMP}] Skipping listener registration - instance should not execute`
  );
}
...
if (shouldExecute) {
  ...
  console.log(
    `🛑 [Content Script][${INSTANCE_TIMESTAMP}] Skipping initialization - instance should not execute`
  );
}
...
function periodicSelfCheck() {
  ...
  try {
    ...
    console.log(
      'Error checking extension context, stopping periodic checks:',
  ...
  if (pingAttempts < MAX_PING_ATTEMPTS || backgroundConnectionVerified) {
    ...
    try {
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          ...
          if (chrome.runtime.lastError) {
            ...
            if (chrome.runtime.lastError.message?.includes('Extension context invalidated')) {
              console.log(
                'Content script context invalidated during heartbeat. Stopping further operations.'
              );
              isContextInvalidated = true;
              if (periodicCheckIntervalId !== undefined) {
                clearInterval(periodicCheckIntervalId);
      ...
      console.log(
        'Error sending heartbeat message, stopping periodic checks:',
        error instanceof Error ? error.message : String(error)
      );
    ...
    console.log(
      `Background connection could not be verified after ${MAX_PING_ATTEMPTS} attempts. Stopping heartbeat checks.`
    );
    if (periodicCheckIntervalId !== undefined) {
      clearInterval(periodicCheckIntervalId);
      periodicCheckIntervalId = undefined;
...
// ONLY if this instance should execute
if (shouldExecute && periodicCheckIntervalId === undefined) {
  // Ensure it's not set multiple times if script re-runs somehow
...
async function handleRunCode(code: string, sendResponse: (response: any) => void) {
  try {
    ...
    const runButton = document.querySelector(
      'button.goog-button.run-button[title="Run script (Ctrl+Enter)"]'
    );
...
function extractInspectorTreeAsJSON(explorer: Element): any {
  ...
  if (!body || body.children.length === 0) {
    ...
    if (colonIndex > -1) {
      ...
      const bandSummaryMatch = value.match(
        /^"([^"]+)",\s*(\w+),\s*(EPSG:\d+),\s*(\d+)x(\d+)\s*px$/
      );
      if (bandSummaryMatch) {
        return {
  ...
  try {
    ...
    if (!inspectPanel) {
      ...
      sendResponse({
        success: false,
        error:
          'Inspector panel not found. Please ensure the Inspector tab is open in Earth Engine.',
      });
      return;
    }
    ...
    if (panelText.includes('Click on the map')) {
      ...
      sendResponse({
        success: false,
        error:
          'Inspector is empty or still loading. Please click on the map first, then wait 1-3 seconds for data to load before calling this tool again.',
      });
      return;
    }
    ...
    console.log(
      `📍 [InspectMap] Extracted coordinates: lng=${inspectedCoords.lng}, lat=${inspectedCoords.lat}`
    );
    ...
    const objectsSection = Array.from(
      inspectPanel.querySelectorAll('.inspect-view.inspect-object')
    );
    ...
    if (
      ...
      objectsData.every((obj) => !obj.data || Object.keys(obj.data).length === 0)
    ) {
      suggestion =
        'Objects section appears empty or not expanded. To get CRS/EPSG information, manually click on "Objects" in the Inspector panel to expand it, then call this tool again.';
...
function notifyBackgroundScriptLoaded() {
  ...
  try {
    if (
      ...
      console.warn(
        'Extension context invalidated, cannot notify background script in notifyBackgroundScriptLoaded'
      );
      isContextInvalidated = true;
      return;
    }
    ...
    chrome.runtime.sendMessage({ type: 'CONTENT_SCRIPT_LOADED' }, (response) => {
      if (chrome.runtime.lastError) {
        ...
        if (connectionRetries < MAX_CONNECTION_RETRIES) {
          ...
          connectionRetries++;
          console.log(
            `Retrying connection to background script in ${delay}ms (attempt ${connectionRetries}/${MAX_CONNECTION_RETRIES})...`
          );

          setTimeout(notifyBackgroundScriptLoaded, delay);
        } else {
          console.error(
            `Failed to connect to background script after ${MAX_CONNECTION_RETRIES} attempts`
          );
        }
        return;
      }
  ...
  try {
    ...
    if (!editorUpdated) {
      ...
      sendResponse({
        success: false,
        error:
          'Failed to update editor content. Could not access the Earth Engine Code Editor. Try manually updating the code.',
      });
      return;
    }
  ...
  try {
    ...
    try {
      ...
      if (result.success && result.content !== undefined) {
        ...
        successMethod = 'Page context injection';
        console.log(
          `Successfully read editor content via page context (${scriptContent.split('\n').length} lines)`
        );
      } else {
        console.warn('Page context method failed:', result.error);
      }
        ...
        if (mainEditor) {
          ...
          if (aceLines.length > 0) {
            ...
            if (textLayer) {
              ...
              scriptContent = (textLayer.innerText || textLayer.textContent || '').replace(
                /\u00a0/g,
                ''
              );
    ...
    console.log(
      `Editor content read successfully using method: ${successMethod}, ${lineCount} lines`
    );
    sendResponse({
      success: true,
  ...
  try {
    ...
    if (!eeConsole) {
      ...
      sendResponse({
        success: false,
        error:
          'Console element not found. Make sure you are on the Google Earth Engine code editor page.',
      });
      return;
    }
    ...
    consoleLogElements.forEach((logElement, index) => {
      try {
        ...
        const visualElements = logElement.querySelectorAll(
          'canvas, svg, img, iframe, [class*="chart"]'
        );
        ...
        if (visualElements.length > 0) {
          ...
          const titleElement = logElement.querySelector(
            '[class*="title"], h3, h4, h5, .chart-title'
          );
        ...
        if (hasVisualContent) {
          ...
          logElement.classList.contains('error') ||
          message.toLowerCase().includes('error')
        ) {
          type = 'error';
        } else if (
          logElement.classList.contains('warning') ||
    ...
    layerItems.forEach((item, index) => {
      try {
        ...
        const name = nameElement
          ? nameElement.textContent?.trim() || `Layer ${index + 1}`
          : `Layer ${index + 1}`;
        ...
        const visibleAttr =
          visibilityEl?.getAttribute('aria-checked') || visibilityEl?.getAttribute('checked');
        const hiddenClass =
          item.classList.contains('hidden') || item.classList.contains('layer-hidden');
        ...
        const opacityEl = item.querySelector(
          '.opacity-slider, input[type="range"], .opacity-value'
        );
        let opacity = 1.0; // Default to full opacity

        if (opacityEl) {
          const opacityValue =
            opacityEl.getAttribute('value') ||
 ...
 */
async function handleExecuteClickByCoordinates(
  x: number,
  y: number,
 ...
 */
async function handleClickBySelector(
  selector: string,
...
async function handleClickByRefId(refId: string, sendResponse: (response: any) => void) {
  ...
  try {
    if (
      ...
      console.warn(
        '[ClickByRefIdTool - CS] Extension context appears to be invalidated. Aborting.'
      );
    ...
    if (element && typeof (element as HTMLElement).click === 'function') {
      ...
      sendResponse({
        success: true,
        message: `Successfully clicked element with refId: ${refId}`,
        refId,
      });
    } else {
      ...
      sendResponse({
        success: false,
        error: `Element with refId '${refId}' not found or not clickable.`,
        refId,
      });
    }
  } catch (error) {
...
async function handleTakeAccessibilitySnapshot(sendResponse: (response: SnapshotResponse) => void) {
  ...
  try {
    ...
    sendResponse({
      success: false,
      error:
        error instanceof Error ? error.message : 'Unknown error in handleTakeAccessibilitySnapshot',
    });
  }
}
...
window.addEventListener('error', (event) => {
  if (
    ...
    console.warn(
      'Global error handler caught extension context invalidation, marking context as invalid'
    );
    isContextInvalidated = true;
