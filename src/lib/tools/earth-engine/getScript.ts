/**
 * Get the current script content from the Google Earth Engine code editor
 * This tool reads the code currently in the Earth Engine editor
 *
 * @returns Promise with success status and script content
 */
 ...
 * Get the current script content from the Google Earth Engine code editor
 *
 * @returns Promise with script content or error message
 */
export async function getScript(): Promise<GetScriptResponse> {
  try {
    ...
    // If not in a browser environment, we can't get the script
    if (env.isNodeJs) {
      return {
        success: false,
