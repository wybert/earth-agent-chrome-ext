...
export async function editCode(
    ...
    const syncedState = shadowWorkspaceSingleton.setFromEditor(tabId, scriptId, fetchResult.content, 'pre-edit sync');
...
export async function insertAtLine(tabId: number, line: number, text: string): Promise<EditorResult> {
    const scriptId = 'current_editor';
    ...
    const syncedState = shadowWorkspaceSingleton.setFromEditor(tabId, scriptId, fetchResult.content, 'pre-insert sync');
    shadowWorkspaceSingleton.markSynced(tabId, syncedState.scriptId);
