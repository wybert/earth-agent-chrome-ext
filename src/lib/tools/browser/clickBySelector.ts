...
export async function clickBySelector(
  params: ClickBySelectorParams
): Promise<ClickBySelectorResponse> {
  ...
  try {
    ...
    if (
      (env.isBackground && typeof chrome !== 'undefined' && chrome.tabs) ||
    ...
    console.error(
      '[Click By Selector] Error during environment detection or execution:',
      errorMessage
    );
    return {
      success: false,
 ...
 */
async function sendClickBySelectorToBackground(
  params: ClickBySelectorParams
): Promise<ClickBySelectorResponse> {
        ...
        (response) => {
          ...
          if (chrome.runtime.lastError) {
            resolve({
              success: false,
              error:
                chrome.runtime.lastError.message || 'Error communicating with background script',
            });
            return;
          }
          resolve(response);
 ...
 */
async function handleBackgroundScriptClickBySelector(
  params: ClickBySelectorParams
): Promise<ClickBySelectorResponse> {
  const { selector, elementDescription } = params;
  ...
  return new Promise<ClickBySelectorResponse>((resolve) => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (chrome.runtime.lastError) {
        console.error(
          '[Click By Selector - Background] Error querying tabs:',
      ...
      chrome.scripting
        .executeScript({
          ...
          func: (selector: string, description?: string) => {
            try {
              console.log(
                `[Content Script - Click By Selector] Looking for element with selector: "${selector}"`
              );
              ...
              const findElementInShadowDoms = (
                ...
                try {
                  ...
                  if (element) {
                    console.log(
                      `[Content Script - Click By Selector] Found element directly:`,
                      element
                    );
                    return element;
                  }
                  ...
                  console.error(
                    `[Content Script - Click By Selector] Error in direct querySelector:`,
                    error
                  );
                }
                ...
                for (const host of Array.from(allElements)) {
                  if (host.shadowRoot && host.shadowRoot.mode === 'open') {
                    console.log(
                      `[Content Script - Click By Selector] Searching in shadow DOM of:`,
                      host
                    );
                    const elementInShadow = findElementInShadowDoms(host.shadowRoot, selector);
                    if (elementInShadow) {
                      console.log(
                        `[Content Script - Click By Selector] Found element in shadow DOM:`,
                        elementInShadow
                      );
              ...
              if (!element) {
                console.error(
                  `[Content Script - Click By Selector] Element not found with selector: "${selector}"`
                );
                return {
                  success: false,
              ...
              console.log(
                `[Content Script - Click By Selector] Element details: tagName=${element.tagName}, id=${element.id || 'none'}, class=${element.className || 'none'}`
              );
        ...
        .then((results) => {
          if (chrome.runtime.lastError) {
            console.error(
              '[Click By Selector - Background] Script execution error:',
              chrome.runtime.lastError.message
            );
            resolve({
              success: false,
 ...
 */
async function executeClickBySelectorInPage(
  selector: string,
  description?: string
): Promise<ClickBySelectorResponse> {
  try {
    console.log(
      `[Content Script - Click By Selector Direct] Looking for element with selector: "${selector}"`
    );
