...
async function captureDirectSnapshot(): Promise<SnapshotResponse> {
  try {
    ...
    if (
      ...
      console.warn(
        '[SnapshotTool] Extension context appears to be invalidated. Aborting direct snapshot.'
      );
      return {
        success: false,
  ...
  if (node) {
    ...
    if (shouldProcessChildren(element)) {
      ...
      for (const child of Array.from(element.children)) {
        const childNodes = buildAccessibilityTree(
          child,
          refCounter,
          processedElements,
          maxDepth,
 ...
 */
function createAccessibilityNode(
  element: Element,
  refCounter: { value: number }
...
function getAccessibleName(element: Element): string {
  try {
    ...
    if (error instanceof Error && error.message.includes('Extension context invalidated')) {
      console.warn(
        '[SnapshotTool] Context invalidated while getting accessible name for element:',
        element,
        error.message
      );
    } else {
...
function getTextContent(element: Element): string {
  try {
      ...
      interactiveElements.forEach((el) => {
        try {
          ...
          if (
            ...
            removeError.message.includes('Extension context invalidated')
          ) {
            console.warn(
              '[SnapshotTool] Context invalidated while removing child from cloned node during getTextContent:',
              el,
              removeError.message
            );
          } else {
            console.warn(
              '[SnapshotTool] Error removing child from cloned node during getTextContent:',
              el,
              removeError
            );
          }
    ...
    if (error instanceof Error && error.message.includes('Extension context invalidated')) {
      console.warn(
        '[SnapshotTool] Context invalidated during getTextContent for element:',
        element,
        error.message
      );
    } else {
