...
export const TOOL_CATALOG: ToolCatalogItem[] = [
  ...
  },
  {
    key: 'dateTime',
    label: 'Date/Time',
    description: 'Get current date/time (optional time zone)',
    kind: 'read',
  },
  {
    key: 'wait',
    label: 'Wait',
    description: 'Wait for specified seconds (for long-running operations)',
    kind: 'read',
  },
  ...
  // Simplified code editing tools (Claude Code-style)
  {
    key: 'readCode',
    label: 'Read Code',
    description: 'Read current code from Earth Engine editor',
    kind: 'read',
  },
  {
    key: 'editCode',
    label: 'Edit Code',
    description: 'Edit code using old_string/new_string replacement',
    kind: 'write',
  },
  {
    key: 'writeCode',
    label: 'Write Code',
    description: 'Overwrite entire editor content',
    kind: 'write',
  },
  ...
  // Earth Engine tools
  {
    key: 'geeDocs',
    label: 'GEE Docs',
    description: 'Search GEE datasets (official/community) and API documentation',
    kind: 'read',
  },
  {
    key: 'runCurrentCode',
    label: 'Run Code',
    description: 'Execute current code in editor',
    kind: 'write',
  },
  ...
  {
    key: 'clickAtScreenPosition',
    label: 'Click (Screen)',
    description: 'Click at screen x/y position',
    kind: 'read',
  },
  ...
  // Earth Engine state tools
  {
    ...
    description: 'Get Earth Engine console output',
    kind: 'read',
  },
  {
    key: 'getMapScreenPosition',
    label: 'Map Position',
    description: 'Get map screen position and bounds',
    kind: 'read',
  },
  {
    key: 'getInspectorOutput',
    label: 'Inspector Output',
    description: 'Get map inspector output',
    kind: 'read',
  },
  {
    key: 'clearMapInspectorAndConsole',
    label: 'Clear All',
    description: 'Clear map, inspector, and console',
    kind: 'write',
  },
];
...
export function inferBaseModeFromTools(tools: ToolKey[]): 'ask' | 'do' {
  const writeKeys = new Set<ToolKey>(
    TOOL_CATALOG.filter((t) => t.kind === 'write').map((t) => t.key)
  );
  ...
  return raw
    .map((p: any) => {
      ...
      const tools = Array.isArray(p.tools)
        ? (p.tools.filter((t: any) => typeof t === 'string') as ToolKey[])
        : [];
      return {
        id: p.id,
        name: p.name,
